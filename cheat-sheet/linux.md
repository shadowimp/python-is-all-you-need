...
ps -ef | grep 9916  | cut -c 9-15 | xargs kill -9
...
# 脚本
##############################################################################
awk 'NR == 10' Iris.txt  # 取Iris.txt 文件的第10行


tar -zcvf test.tar.gz test # 压缩 test文件
tar -zxvf file.tar.gz      #解压缩file.tar.gz 文件到当前目录

tar -tf test.tar.gz    # 在不解压的情况下 ， 查看文件内容
